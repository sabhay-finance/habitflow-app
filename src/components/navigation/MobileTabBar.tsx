    ...
    <>
      ...
      <div className="fixed bottom-20 right-4 sm:bottom-8 sm:right-8 z-40">
        <motion.button
          ...
          className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-brand-600 to-violet-500 text-white shadow-xl shadow-brand-500/40 flex items-center justify-center focus:outline-none focus:ring-4 focus:ring-brand-400/40"
      ...
      {/* Bottom Tab Bar */}
      <nav className="fixed bottom-0 left-0 right-0 z-30 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-t border-slate-200/80 dark:border-slate-800/80 pb-safe transition-colors">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className="relative flex flex-col items-center justify-center w-14 h-12 rounded-xl text-slate-500 dark:text-slate-400 focus:outline-none transition-colors select-none"
              >
                {isActive && (
                  <motion.div
